...
{
    public const META_TYPE_STRING = 'string';

    public const META_TYPE_BOOLEAN = 'boolean';

    public const META_TYPE_INTEGER = 'integer';

    public const META_TYPE_FLOAT = 'float';

    public function addMeta($name, $type, $default, $comment, $length = 0)
    {
        if (! $this->hasField($name)) {
            call_user_func_array([$this->db, 'addColumn'], func_get_args());
        }
    }
