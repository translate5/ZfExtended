...
EOD;
    ...
    private function addNonEmptyRow(Worksheet $worksheet, $row)
    {
        ...
        foreach ($cells as $cell) {
            $rowXML .= "\t".$this->applyStyleAndGetCellXML($cell, $rowStyle, $rowIndex, $cellIndex, $worksheet->getExternalSheet()->getIndex());
            $cellIndex++;
        }
     ...
     */
    private function applyStyleAndGetCellXML($cell, Style $rowStyle, $rowIndex, $cellIndex, $sheetIndex = 0)
    {
     ...
     * @return string
     */
    protected function setColumnMaxCharacters($columnIndex, $text, $sheetIndex = 0)
    {
     ...
     * @throws InvalidArgumentException If the given value cannot be processed
     */
    private function getCellXML($rowIndex, $cellNumber, $cell, $styleId, $sheetIndex = 0)
    {
    ...
    public function close(Worksheet $worksheet, Style $defaultStyle = null)
    {
        ...
        if (count($sheet->getColumnDimensions())) {
            ...
            i(['index' => $sheet->getIndex(), $this->columnsMaxTextLength[$sheet->getIndex()]], 'a');
